...
vnode_t *tmpfs_create_vnode(const char *name, vnode_t *parent, enum vnode_type type) {
    ...
    inter_tmpfs_t *internal = (inter_tmpfs_t *)kmalloc(sizeof(inter_tmpfs_t));
    v->internal = internal;
    internal->file = 0;
    internal->vnode = v;
    internal->parent = parent;
    str_cpy(internal->name, name);
    internal->type = type;
    list_init(&internal->d_siblings);
    list_init(&internal->d_subdirs);
    if (parent != 0)
    {
        list_push(&((inter_tmpfs_t *)(parent->internal))->d_subdirs, (list_head *)v);
    }
    return v;
}
...
int tmpfs_lookup(vnode_t* dir_node, vnode_t** v_tar, const char* component_name) {
    ...
    inter_tmpfs_t *parent = (inter_tmpfs_t *)(dir_node->internal);
    for (list_head *subdir = parent->d_subdirs.next; subdir != &parent->d_subdirs; subdir = subdir->next)
    {
        inter_tmpfs_t *child = (inter_tmpfs_t *)list_entry(subdir, struct vnode, internal);
...
int tmpfs_create(vnode_t* dir_node, vnode_t** target, const char* file_name) {
    ...
    inter_tmpfs_t *inter = (inter_tmpfs_t *)((*target)->internal);
    inter->file = fd;
