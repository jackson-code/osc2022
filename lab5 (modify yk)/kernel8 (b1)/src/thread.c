...
void threadSchedule(){
	...
	if(rq.beg==rq.end){
		...
		do{	// executing thread(beg) is moved to the end of the queue, 
			// the next of executing thread be the beg
			rq.end->next=rq.beg;
			rq.end=rq.beg;
			rq.beg=rq.beg->next;
			rq.end->next=0;
...
/*
 1. alloc a page to the thread
 2. set the sp at the highest addr of the page
 3. push into queue for schedule
*/
Task* thread_create(void* func){
	Task* new_task=(Task*)kalloc(TASKSIZE);
	if((unsigned long)new_task%TASKSIZE){//aligned
		uart_printf("new_task isn't aligned!!\n");
		while(1){}
	}
	
	// user stack of thread put in the end of the page which buddy sys alloc
...
/*--------------------------------------------*/
/*
 TASKEXIT thread were removed from RQ
*/
void zombiesKill(){//called by idle()
	Task* tar = rq.beg;
	while(1){
		while(tar->next && (tar->next->task_state == EXIT)){
