...
int threshold = PHYSICAL_NAND_NUM - 2;
...
static int ftl_write(const char* buf, size_t lba_rnage, size_t lba)
{
    ...
    if (L2P[lba] != INVALID_PCA) {
        P2L[pca2P2Lidx(L2P[lba])] = INVALID_LBA;
        valid_count[pca2P2Lidx(L2P[lba]) / PAGE_PER_BLOCK]--;
    }
    unsigned int pca = get_next_pca();
    P2L[pca2P2Lidx(pca)] = lba;
    L2P[lba] = pca;
    nand_write(buf, pca);
...
int pca2P2Lidx(unsigned int pca) {
    return (pca >> 16) * PAGE_PER_BLOCK + (pca & 0x0000ffff);
}

int GC() {
    int i, min_valid_count, erase_block, j;
    for (i = 1, erase_block = 0, min_valid_count = valid_count[erase_block]; i < PHYSICAL_NAND_NUM; i++) {
        if (valid_count[i] < min_valid_count) {
            min_valid_count = valid_count[i];
            erase_block = i;
        }
    }
    if (min_valid_count == PAGE_PER_BLOCK) {
        printf("there's no enough room\n");
    }

    int lba;
    char* tmp_buf = calloc(512, sizeof(char));
    while (valid_count[erase_block] > 0) {
        for (i = 0; i < PAGE_PER_BLOCK; i++) {
            if ((lba = P2L[erase_block * PAGE_PER_BLOCK + i]) != INVALID_LBA) {
                ftl_read(tmp_buf, lba);
                ftl_write(tmp_buf, 0, lba);
            }
        }
    }
    free(tmp_buf);
    return nand_erase(erase_block);
}
...
static int ssd_do_write(const char* buf, size_t size, off_t offset)
{
    ...
    tmp_lba = offset / 512;
    tmp_lba_range = (offset + size - 1) / 512 - (tmp_lba)+1;    // count of blk
    tmp_buf = calloc(512, sizeof(char));

    size_beg = offset % 512;                // have to write 512 - size_beg in first blk
    size_end = (size_beg + size) % 512;     // have to write size_end in last blk
    // unused variable
    process_size = 0;
    remain_size = size;
    ...
    {
        // TODO
        if (idx == 0) {                                     // first blk maybe unaligned
            if (L2P[tmp_lba + idx] == INVALID_PCA) {        // empty blk
                memset(tmp_buf, 0, sizeof(char) * 512);
            }
            else {
                ftl_read(tmp_buf, tmp_lba + idx);           // original data on blk
            }
            memcpy(tmp_buf + size_beg, buf, 512 - size_beg);// conbine new & original data
        }
        else if (idx == tmp_lba_range - 1 && size_end != 0) {// last blk maybe unaligned
            if (L2P[tmp_lba + idx] == INVALID_PCA) {         // empty blk
                memset(tmp_buf, 0, sizeof(char) * 512);
            }
            else {
            ...
            memcpy(tmp_buf, buf + size - size_end, size_end);
        }
        else {                                                  // blk between first & last blk
            memcpy(tmp_buf, buf - size_beg + idx * 512, 512);
        }

        ftl_write(tmp_buf, tmp_lba_range, tmp_lba + idx);
        printf("free_block_number:%d lba:%d\n", free_block_number, curr_pca.fields.lba);
        
        if (curr_pca.fields.lba == 9 && free_block_number == 1) {
            GC();
        }
    }
    free(tmp_buf);
