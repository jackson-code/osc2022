#include "thread.h"

static RQ rq;
static int task_cnter;

void threadSwitch(){
	asm volatile("\
_threadSwitch:\n\
		stp x19, x20, [x0, 16 * 0]\n\
		stp x21, x22, [x0, 16 * 1]\n\
		stp x23, x24, [x0, 16 * 2]\n\
		stp x25, x26, [x0, 16 * 3]\n\
		stp x27, x28, [x0, 16 * 4]\n\
		stp x29, x30, [x0, 16 * 5]\n\
		mov x9, sp\n\
		str x9, [x0, 16 * 6]\n\
		\n\
		ldp x19, x20, [x1, 16 * 0]\n\
		ldp x21, x22, [x1, 16 * 1]\n\
		ldp x23, x24, [x1, 16 * 2]\n\
		ldp x25, x26, [x1, 16 * 3]\n\
		ldp x27, x28, [x1, 16 * 4]\n\
		ldp x29, x30, [x1, 16 * 5]\n\
		ldr x9, [x1, 16 * 6]\n\
		mov sp, x9\n\
		\n\
		msr tpidr_el1, x1\n\
		\n\
		ret\n\
	"::);
}


/*
 context switch
*/
int threadSchedule(){
	uart_puts("threadSchedule\n");

	if(!rq.beg){
		uart_puts("rq is empty!!\n");
		//while(1){}
		return 0;
	}
	else{
		unsigned long count = 0;
		do{	// executing thread(beg) is moved to the end of the queue, 
			// the next of executing thread be the beg
			rq.end->next=rq.beg;
			rq.end=rq.beg;
			rq.beg=rq.beg->next;
			rq.end->next=0;
			count++;
		}while(rq.beg->status == TASKEXIT && count != rq.count);//ignore abnormal task

		//uart_puts("%d %x %x %x\n",rq.beg->id,rq.beg->context[10],rq.beg->context[11],rq.beg->context[12]);
		//uart_puts("%d -> %d\n",rq.end->id,rq.beg->id);

		if (count == rq.count) {
			asm volatile("\
				mov x1, %0\n\
				mrs x0, tpidr_el1\n\
				bl _threadSwitch\n\
				"::"r"(rq.caller));	// /switch back to caller
		}
		else {
			// get next thread's addr (x1)
			// get executing thread's reg set (x0)
			asm volatile("\
				mov x1, %0\n\
				mrs x0, tpidr_el1\n\
				bl _threadSwitch\n\
				"::"r"(rq.beg));//only use bl to avoid stack usage
		}
		return 1;
	}
}

Task* thread_create(void* func){
	Task* new_task=(Task*)kmalloc(TASKSIZE);
	if((unsigned long)new_task%TASKSIZE){//aligned
		uart_puts("new_task isn't aligned!!\n");
		while(1){}
	}

	rq.count++;
	
	uart_puts("thead_create, assign fp, lr, sp, \t");
	new_task->context[10]=(unsigned long)new_task+TASKSIZE;//fp
	new_task->context[11]=(unsigned long)func;//lr
	new_task->context[12]=(unsigned long)new_task+TASKSIZE;//sp
	new_task->id=task_cnter++;
	new_task->status=0;
	new_task->a_addr=new_task->a_size=new_task->child=0;
	new_task->next=0;

	uart_puts("push into RQ\n");
	if(rq.beg){
		rq.end->next=new_task;
		rq.end=rq.end->next;
	}else{
		rq.beg=rq.end=new_task;
	}

	return new_task;
}

/*--------------------------------------------*/

/*
 remove all EXIT threads from RQ and memory
*/
void zombiesKill(){//called by idle()
	Task* tar = rq.beg;
	
	while(1){
		while(tar->next && (tar->next->status & TASKEXIT)){
			Task* tmp=tar->next->next;
			//kfree((unsigned long)(tar->next));
			kfree(tar->next);
			tar->next = tmp;
			rq.count--;
		}

		if(!tar->next){
			if (tar->status & TASKEXIT) {
				kfree(tar);
				rq.count--;
				rq.beg = rq.end = 0;
			}
			else
			{
				rq.beg = rq.end = tar;
			}
			break;
		}else{
			tar = tar->next;
		}
	}
	
/*
	unsigned long seen = 0;
	while(seen <= rq.count){
		while(tar && (tar->status & TASKEXIT)){
			Task* tmp=tar->next;
			//kfree((unsigned long)(tar->next));
			kfree(tar);
			tar = tmp;
			rq.count--;
			seen++;
		}

		rq.beg = tar;
		
		if(!tar){
			rq.end = 0;
			break;
		}else{
			tar = tar->next;
			seen++;
		}
	}
	*/
}

void taskUpdate(Task* p,Task* c){
	p->status^=TASKFORK;
	p->child=c->id;

	Task* tmp=c->next;
	char* src=(char*)p;
	char* dst=(char*)c;
	for(int i=0;i<TASKSIZE;++i){//task copy
		*dst=*src;
		dst++;
		src++;
	}

	c->id=p->child;
	uart_puts("Please enter app load address (Hex): ");
	//c->a_addr=uart_get_hex(1);
	c->child=0;
	c->next=tmp;

	long k_delta=(long)c-(long)p;
	long a_delta=(long)c->a_addr-(long)p->a_addr;
	c->context[10]+=k_delta;//kernel fp
	c->context[12]+=k_delta;//kernel sp
	c->context[14]+=a_delta;//elr_el1
	c->context[15]+=a_delta;//sp_el0
	c->context[45]+=a_delta;//user fp
	c->context[46]+=a_delta;//user lr

	src=(char*)(p->context[15]);
	dst=(char*)(c->context[15]);
	for(int i=0,ii=p->a_addr+p->a_size-(p->context[15]);i<ii;++i){//program copy
		*dst=*src;
		dst++;
		src++;
	}
}

void doFork(){//called by idle()
	Task* tar=rq.beg->next;
	while(tar){
		if((tar->status)&TASKFORK){
			Task* child=thread_create(0);
			taskUpdate(tar,child);
		}
		tar=tar->next;
	}
}

void idle(){
	uart_puts("idle()\n");
	int queue_not_empty = 1;
	while(queue_not_empty){
		//uart_getc();
		zombiesKill();
		//doFork();
		queue_not_empty = threadSchedule();
	}
}
void delay(int period) {
    while (period--);
}

/*--------------------------------------------*/

int tidGet(){
	Task* cur;
	asm volatile("mrs %0, tpidr_el1\n":"=r"(cur):);
	return cur->id;
}

void exec(char* path,char** argv){//will not reset sp...
	unsigned long a_addr;
	uart_puts("Please enter app load address (Hex): ");
	//a_addr=uart_get_hex(1);
	//loadApp_with_argv(path,a_addr,argv,&(rq.beg->a_addr),&(rq.beg->a_size));
	exit();
}

/*
 thread is marked EXIT 
*/
void exit(){
	uart_puts("exit()\n");

	Task* cur;
	asm volatile("mrs %0, tpidr_el1\n":"=r"(cur):);
	cur->status |= TASKEXIT;
	threadSchedule();

	while(1){
		uart_puts("exit() failed!!\n");
	}
}

int fork(){
	rq.beg->status|=TASKFORK;
	threadSchedule();
	return rq.beg->child;
}

/*--------------------------------------------*/


void foo1(){
	for(int i = 0; i < 2; ++i){
		uart_puts("Thread id: ");
		uart_put_int(tidGet());
		uart_puts(", ");
		uart_put_int(i);
		uart_puts("\n");
		//uart_getc();
		delay(1000000);
		threadSchedule();
	}

	exit();
}

void threadTest1(){
	//Task* cur=thread_create(0);//use startup stack (not kernel stack)
	//asm volatile("msr tpidr_el1, %0\n"::"r"((unsigned long)cur));

	// mem for kernel x19~x30
	unsigned long cur = (unsigned long)kmalloc(TASKSIZE);
	asm volatile("msr tpidr_el1, %0\n"::"r"(cur));
	rq.caller = cur;

	for(int i=0;i<3;++i){
		thread_create(foo1);
	}

	idle();

	kfree((void *)cur);
	uart_puts("threadTest finish\n");
}

void foo2(){
	char* argv[]={"argv_test","-o","arg2",0};
	exec("app1",argv);
}

void threadTest2(){
	Task* cur=thread_create(0);//use startup stack (not kernel stack)
	asm volatile("msr tpidr_el1, %0\n"::"r"((unsigned long)cur));//TODO

	thread_create(foo2);

	idle();
}
