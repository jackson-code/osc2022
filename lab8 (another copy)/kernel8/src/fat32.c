...
int fat32_setup_mount(filesystem_t *tmpfs, struct mount *_mount) {
    ...
    */
    ...
    char buf[BLOCK_SIZE];
    sd_read_block(parti_blk_idx, buf);    // the boot sector is the first sector of a partition         
...
int fat32_create(vnode_t* dir_node, vnode_t** file_node, const char* file_name) {
    if (!str_cmp("\0", (*file_node)->name))    // the file's vnode not existing
    {
        ...
        if (ret == -1)
        {
            ...
            while (FAT_cache->table_idx < fat32_meta.sec_size.FAT_region)
            {
                if (FAT_cache->table_idx > 0)                  // skip special file only in first block
                    first_clus = 0;                 
        ...
        for (list_head *sibling = fat32_meta.dir_table_cache->siblings.next; sibling != &fat32_meta.dir_table_cache->siblings; sibling = sibling->next)
        {
            ...
            dir_entry = dir_table_cache->dir_table;
...
int update_dir_entry_size(uint32_t size, const char *name, vnode_t *v) {
    ...
    struct fat32_dir_entry *dir_table = fat32_meta.dir_table_cache->dir_table;
    struct fat32_dir_entry *dir_entry = dir_table;
