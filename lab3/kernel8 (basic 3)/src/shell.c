...
void shell_execute(char *cmd)
{
	if (!str_cmp(cmd, "help")) {
		...
		uart_puts("b123:\t el_user_start(), switch EL1 to EL0, then laod user program, print current time each 2 seconds");
		uart_puts("b2:\t set timer interrupt");
		uart_puts("b3:\t asyn uart read/write by interrupt handler");
	}
